...
My module is the **application form** — the part of UniGate where a logged-in student actually applies to a programme. It is the bridge between Flavja's programme catalogue (where the student picks what to apply to) and the database (where the application is stored for the admin staff to review).

**In one sentence:** I built a three-step form that collects personal and academic information, validates it on both the frontend and the backend, and stores it in the `applications` table linked to the student and the chosen programme.
...
The Sequelize model defining the `applications` table. Personal and academic fields are `NOT NULL`. The four `bachelor_*` fields are nullable because they only apply to Master applications. The `status` column is an ENUM used by Mei's admin dashboard.
...
Three short snippets that show the most important design decisions in this module.

### 1. The "already applied at this university" check on page load

When the page loads, the form fetches both the programme and the student's existing applications in parallel, then compares them. If a conflict exists, the form is blocked before the student can fill anything in.
...
**Why `Promise.all`:** the two requests don't depend on each other, so we fire them in parallel instead of sequentially. This halves the loading time. If the existing-applications request fails, we treat it as an empty list with `.catch()` so the page still loads.

### 2. Conditional Master fields when the programme is a Master's

The form reads `programme.degree_type` once and reuses the same boolean for both rendering and validation. This keeps the two in sync — there is no way for the Bachelor section to be hidden but still required (or vice versa).

```jsx
const isMaster = programme?.degree_type === 'Master';

// In Step2Fields render:
{isMaster && (
  <>
    <SubsectionHeading>Bachelor degree</SubsectionHeading>
    <Field label='Bachelor school name' ... />
    <Field label='Bachelor school country' ... />
    <Field label='Bachelor graduation year' ... />
    <Field label='Bachelor GPA' ... />
  </>
)}

// In validation:
function validateStep2(fields, t, isMaster = false) {
  // ...high school checks for everyone...
  if (isMaster) {
    // ...bachelor checks only for Master applicants...
  }
}
```

**Why one boolean:** rendering and validation must agree — if I show the fields I must require them, and vice versa. Using the same `isMaster` value in both places makes that impossible to get wrong.

### 3. Server-side enforcement of the one-per-university rule

Even though the frontend already checks for a conflict, the backend repeats the check. A user could disable JavaScript or hit the API directly, so we never trust the client. The check uses a Sequelize join to find any existing application whose programme belongs to the same university.
...
if (conflict) {
  return res.status(409).json({
    status: 'error',
    message: `You have already applied to a programme at ${programme.University.name}.`,
  });
}
```

**Why 409:** `409 Conflict` is the correct HTTP status when the request is well-formed but conflicts with the current state of the resource — exactly this case.

---

## Likely questions and answers

**Q: Why three steps instead of one long form?**

> A: Long forms feel overwhelming and produce more drop-offs. Splitting into Personal details, Academic history, and Review breaks the cognitive load into manageable chunks. The progress indicator at the top tells the student exactly where they are. The user can go back to a previous step and edit their answers, with all data preserved — we hold the form state at the page level rather than per step.

**Q: How does the form know whether to show the Bachelor section?**

> A: When the form loads, it fetches the programme details including `degree_type`. We compute `isMaster = programme.degree_type === 'Master'` and use that boolean both in rendering (to show or hide the Bachelor subsection in step 2) and in validation (to require the bachelor fields only for Master applicants). Using the same boolean in both places keeps them in sync.

**Q: Why validate on both the frontend AND the backend?**

> A: Frontend validation is for user experience — it gives instant feedback as the user types or clicks Next, and prevents wasted server requests. Backend validation is for security and integrity — we never trust the client. A user could disable JavaScript or send requests directly to the API. Defence in depth means both layers have to be wrong before bad data reaches the database.

**Q: What happens if a student submits the form but their session expired?**

> A: The submit fails with a 401 from the backend. The frontend catches this and shows a generic error — currently it does not auto-redirect to login, which is a small improvement we could add. The user's form data is preserved in component state, so they do not lose what they typed.

**Q: Why use `Promise.all` when loading the page?**

> A: We need two pieces of data before we can decide what to show: the programme itself and the student's existing applications. The two requests do not depend on each other, so we fire them in parallel instead of sequentially. This halves the loading time. If the existing-applications request fails, we treat it as an empty list with a `.catch()` so the page still loads.

**Q: How is form state managed between steps?**

> A: All form data is held in one `useState` hook at the `Apply.jsx` page level — not split per step. When the user clicks Back, the previous values are still there because we never unmount the state. Each step component is just a controlled UI over the same shared `fields` object, with an `onChange` handler that updates the parent state.

**Q: What is the difference between a 400, 401, 403, 404, and 409 response in this endpoint?**

> A: `400` means the request was malformed or a required field was missing or invalid (e.g. GPA out of range). `401` means the user is not logged in. `403` means the user is logged in but is not allowed (e.g. an admin trying to submit a student application). `404` means the programme they're applying to does not exist. `409` means the request was valid but conflicts with the current state — in this case, they've already applied to that university.

**Q: Why are the `bachelor_*` columns nullable in the database?**

> A: Because they only apply to Master applications. For Bachelor applicants those fields are not collected and are stored as `NULL`. The model defines them as `allowNull: true` with `defaultValue: null`. The backend explicitly sets them to null when the programme is a Bachelor — even if a client tries to send `bachelor_*` fields for a Bachelor application, we ignore them.

---

## What you should be able to demo

1. Log in as a student, click a programme, click Apply.
2. Walk through the three-step form, showing how each step validates before advancing.
3. Apply to a Master programme to demonstrate the Bachelor subsection appearing automatically.
4. After submitting, navigate back to the same university's programme — show the "already applied" state on the programme detail page.
5. Try to apply to a different programme at the same university — show the form is blocked with the conflict message.
